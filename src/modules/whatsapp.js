...
class WhatsAppBot {
    ...
    setupEventHandlers() {
        ...
        this.client.on('message_create', async (message) => {
            logger.debug(`[FLOW] Message received: "${message.body?.substring(0, 50)}..." fromMe: ${message.fromMe}`);
            ...
            try {
                ...
                logger.info(`[FLOW] Processing message from ${contact.name || phoneNumber}: "${message.body}"`);

                // Global rate limiting check
                if (!rateLimiter.checkGlobal()) {
                    logger.warn('[FLOW] Global rate limit exceeded');
                    await message.reply('⚠️ System is experiencing high load. Please try again in a few minutes.');
                    return;
                }
                ...
                if (parsedCommand && parsedCommand.command === 'business') {
                    ...
                    if (!rateLimiter.checkCustomer(phoneNumber)) {
                        ...
                        await message.reply(`⚠️ Rate limit exceeded. You can make ${remaining} more queries. Limit resets at ${resetTime.toLocaleTimeString()}.`);
                        return;
                    }
    ...
    parseCommand(messageBody) {
        ...
        if (messageBody && messageBody !== normalized && (messageBody.includes('\n') || messageBody.includes('\r'))) {
            logger.debug(`[PARSER] Normalized multi-line message: "${messageBody.substring(0, 50)}..." → "${normalized.substring(0, 50)}..."`);
        }
        ...
        logger.debug(`[PARSER] Parsed command: '${parsed.command}' with args: '${parsed.args.substring(0, 50)}${parsed.args.length > 50 ? '...' : ''}'`);
        return parsed;
    }
    ...
    async handleOwnerCommands(message, phoneNumber) {
        ...
        logger.debug(`[OWNER] Recognized business owner: ${business.businessName} (${business.businessId})`);
        ...
        if (message.hasMedia) {
            if (!rateLimiter.checkBusinessOwner(phoneNumber, 'upload')) {
                ...
                await message.reply(`⚠️ Upload rate limit exceeded. You can upload ${remaining} more documents. Limit resets at ${resetTime.toLocaleTimeString()}.`);
                return true;
            }
        ...
        if (!rateLimiter.checkBusinessOwner(phoneNumber)) {
            ...
            const resetTime = new Date(rateLimiter.getResetTime('businessOwner', phoneNumber));
            await message.reply(`⚠️ Rate limit exceeded. You can make ${remaining} more requests. Limit resets at ${resetTime.toLocaleTimeString()}.`);
            return true;
        }
        ...
        switch (parsedCommand.command) {
            ...
            default:
                logger.debug(`[OWNER] Unrecognized command '${parsedCommand.command}' from ${business.businessName}`);
    ...
    async handleRegistration(message, phoneNumber, parsedCommand) {
        ...
        if (!nameValidation.valid) {
            logger.warn(`[REGISTRATION] Invalid business name from ${phoneNumber}: ${nameValidation.error}`);
        ...
        if (validator.detectSuspiciousContent(businessName)) {
            ...
            await message.reply('❌ Business name contains invalid content. Please use a different name.');
            return true;
        }

        logger.info(`[REGISTRATION] Attempting to register business "${businessName}" for ${phoneNumber}`);
        const result = await businessService.registerBusiness(phoneValidation.sanitized, businessName);
        
        if (result.success) {
            logger.success(`[REGISTRATION] Successfully registered business: ${businessName} (ID: ${result.businessId}) for ${phoneNumber}`);
            await message.reply(`🎉 ${result.message}\n\nAvailable commands:\n• !add [text] - Add text knowledge\n• Send documents - Upload files\n• !list - View your knowledge base\n• !delete [id] - Remove knowledge\n• !help - Show all commands`);
        } else {
            logger.error(`[REGISTRATION] Failed to register business for ${phoneNumber}: ${result.message}`);
            await message.reply(`❌ ${result.message}`);
        }
    ...
    async handleMediaUpload(message, business) {
        try {
            logger.info(`[MEDIA] Starting media upload for ${business.businessName} (${business.businessId})`);
            ...
            logger.info(`[MEDIA] Downloaded media: ${media.filename || 'unnamed'}, size: ${media.data.length} bytes`);
            ...
            if (!filenameValidation.valid) {
                logger.warn(`[MEDIA] Invalid filename from ${business.businessName}: ${filenameValidation.error}`);
                await message.reply(`❌ ${filenameValidation.error}`);
                return true;
            }

            // Validate file type
            const fileTypeValidation = validator.validateAndSanitize('fileType', filenameValidation.sanitized);
            if (!fileTypeValidation.valid) {
                logger.warn(`[MEDIA] Invalid file type from ${business.businessName}: ${fileTypeValidation.error}`);
            ...
            if (!fileSizeValidation.valid) {
                logger.warn(`[MEDIA] Invalid file size from ${business.businessName}: ${fileSizeValidation.error}`);
            ...
            if (fileExt === '.pdf') {
                ...
                await message.reply('🖼️ Image received! Please add a description by replying with: !add [description]');
            ...
            if (!content.trim()) {
                logger.warn(`[MEDIA] Empty content extracted from ${filename} for ${business.businessName}`);
            ...
            if (!contentValidation.valid) {
                logger.warn(`[MEDIA] Invalid extracted content from ${business.businessName}: ${contentValidation.error}`);
            ...
            if (validator.detectSuspiciousContent(contentValidation.sanitized)) {
                logger.warn(`[MEDIA] Suspicious content detected in document from ${business.businessName}`);
            ...
            if (result.success) {
                logger.success(`[MEDIA] Document successfully stored: ${result.knowledgeId} for ${business.businessName}`);
                await businessService.updateKnowledgeCount(business.ownerPhone);
                await message.reply(`✅ ${result.message}`);
            } else {
                logger.error(`[MEDIA] Failed to store document for ${business.businessName}: ${result.message}`);
                await message.reply(`❌ ${result.message}`);
            }
    ...
    async handleTextKnowledge(message, business, parsedCommand) {
        ...
        if (!contentValidation.valid) {
            logger.warn(`[TEXT] Invalid content from ${business.businessName}: ${contentValidation.error}`);
        ...
        logger.info(`[TEXT] Adding ${content.length} characters of text knowledge for ${business.businessName}`);
        ...
        if (result.success) {
            logger.success(`[TEXT] Text knowledge added: ${result.knowledgeId} for ${business.businessName}`);
            await businessService.updateKnowledgeCount(business.ownerPhone);
            await message.reply(`✅ ${result.message}\n"${content.substring(0, 100)}${content.length > 100 ? '...' : ''}"`);
        } else {
            logger.error(`[TEXT] Failed to add text knowledge for ${business.businessName}: ${result.message}`);
            await message.reply(`❌ ${result.message}`);
        }
    ...
    async handleListKnowledge(message, business) {
        ...
        if (entries.length === 0) {
            ...
            await message.reply(`📚 Your Knowledge Base (${business.businessName}):\n\nNo entries yet. Use !add [text] or send documents to get started!`);
            return true;
        }
    ...
    async handleDeleteKnowledge(message, business, parsedCommand) {
        ...
        if (!idValidation.valid) {
            logger.warn(`[DELETE] Invalid knowledge ID from ${business.businessName}: ${idValidation.error}`);
            await message.reply(`❌ ${idValidation.error}`);
            return true;
        }

        const knowledgeId = idValidation.sanitized;
        logger.info(`[DELETE] Attempting to delete knowledge ${knowledgeId} for ${business.businessName}`);

        const result = await knowledgeService.deleteKnowledge(business.businessId, knowledgeId);
        
        if (result.success) {
            logger.success(`[DELETE] Successfully deleted knowledge ${knowledgeId} for ${business.businessName}`);
            await businessService.updateKnowledgeCount(business.ownerPhone, -1);
            await message.reply(`✅ ${result.message}`);
        } else {
            logger.warn(`[DELETE] Failed to delete knowledge ${knowledgeId} for ${business.businessName}: ${result.message}`);
            await message.reply(`❌ ${result.message}`);
        }
...
- Use descriptive text for better customer responses`;
    ...
    async handleClearCache(message, business) {
        try {
            ...
            await message.reply(`🗑️ Cleared ${clearedCount} cached responses for ${business.businessName}. Fresh queries will now search updated knowledge base.`);
            return true;
        } catch (error) {
    ...
    async handleBusinessQuery(message, businessId, parsedCommand) {
        try {
            ...
            logger.info(`[QUERY] Sending response for business ${businessId} (${response.length} characters)`);
            await message.reply(response);
