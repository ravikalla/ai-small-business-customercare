...
class RetryManager {
  ...
  static async withCircuitBreaker(operation, options = {}) {
    const { failureThreshold = 5, resetTimeoutMs = 60000, operationName = 'operation' } = options;
    ...
    try {
      ...
      if (breaker.state === 'HALF_OPEN') {
        ...
        logger.info(
          `[CIRCUIT] ${operationName}: Circuit breaker CLOSED after successful operation`
        );
      }
      ...
      if (breaker.failureCount >= failureThreshold) {
        breaker.state = 'OPEN';
        logger.error(
          `[CIRCUIT] ${operationName}: Circuit breaker OPENED after ${breaker.failureCount} failures`
        );
      }
