/**
 * Knowledge Data Access Layer for Supabase
...
class KnowledgeModel {
    ...
    async create(knowledgeData) {
        try {
            ...
            const { data, error } = await RetryManager.withRetry(async () => {
                return await client
                    .from(this.tableName)
                    .insert([{
    ...
    async deleteByBusinessId(businessId) {
        try {
            ...
            logger.debug(`[KNOWLEDGE_MODEL] Deleted ${data.length} knowledge entries for business ${businessId}`);
