...
class AIService {
    ...
    async generateResponse(query, businessId) {
        try {
            ...
            logger.debug(`[AI] Built context: ${context.length} characters from ${relevantDocs.length} sources`);
...
Response:`;
            ...
            const completion = await RetryManager.withCircuitBreaker(async () => {
            ...
            logger.info(`[AI] Generated response for business ${businessId}: ${response.length} characters`);
