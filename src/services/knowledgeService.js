...
class KnowledgeService {
    ...
    async addTextKnowledge(businessId, businessName, content, metadata = {}) {
        try {
            logger.info(`[KNOWLEDGE] Adding text knowledge for ${businessName} (${content.length} chars)`);
            ...
            logger.success(`[KNOWLEDGE] Successfully added text knowledge: ${knowledgeId} for ${businessName}`);
            return {
                success: true,
                knowledgeId,
    ...
    async addDocumentKnowledge(businessId, businessName, filename, content, fileType, metadata = {}) {
        try {
            ...
            logger.success(`[KNOWLEDGE] Successfully added document knowledge: ${knowledgeId} for ${businessName}`);
            return {
                success: true,
                knowledgeId,
        ...
        try {
            ...
            try {
                ...
                if (vectorResult.success) {
                    logger.success(`[KNOWLEDGE] Deleted ${vectorResult.deletedCount} vector chunks for ${knowledgeId}`);
                } else {
                    logger.warn(`[KNOWLEDGE] Failed to delete vectors for ${knowledgeId}: ${vectorResult.error}`);
                }
            } catch (vectorError) {
    ...
    async deleteAllBusinessKnowledge(businessId) {
        try {
            ...
            logger.info(`[KNOWLEDGE] Deleted ${deletedIds.length} knowledge entries for business ${businessId}`);
            ...
            try {
                ...
                if (vectorResult.success) {
                    ...
                    logger.warn(`[KNOWLEDGE] Failed to delete vectors for business ${businessId}: ${vectorResult.error}`);
                }
            } catch (vectorError) {
