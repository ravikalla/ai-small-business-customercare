...
class BusinessService {
    ...
    generateBusinessId(businessName) {
        const prefix = businessName.toLowerCase().replace(/[^a-z0-9]/g, '').substring(0, 8);
    ...
    async registerBusiness(phoneNumber, businessName) {
        ...
        try {
            ...
            logger.success(`[BUSINESS] Successfully registered: ${businessName} (${businessId}) for ${phoneNumber}`);
            return {
                success: true,
                businessId,
                businessName,
    ...
    async updateKnowledgeCount(phoneNumber, increment = 1) {
        try {
            ...
            if (result) {
                logger.debug(`[BUSINESS] Updated knowledge count for ${result.businessName}: ${increment > 0 ? '+' : ''}${increment}`);
            } else {
                logger.warn(`[BUSINESS] Cannot update knowledge count: business not found for ${phoneNumber}`);
            }
            return result;
        } catch (error) {
    ...
    getBusinessByOwnerSync(phoneNumber) {
        ...
        logger.warn('[BUSINESS] Using deprecated synchronous getBusinessByOwner - please update to async');
