...
class VectorService {
    ...
    async generateEmbedding(text) {
        ...
        const embedding = await RetryManager.withRetry(async () => {
            logger.debug(`[VECTOR] Generating embedding for ${text.length} characters`);
            const response = await this.openai.embeddings.create({
                model: 'text-embedding-3-small',
                input: text
            });
            logger.debug(`[VECTOR] Successfully generated embedding with ${response.data[0].embedding.length} dimensions`);
    ...
    async storeDocument({ businessId, businessName, filename, content, metadata }) {
        try {
            ...
            for (let i = 0; i < chunks.length; i++) {
                logger.debug(`[VECTOR] Processing chunk ${i + 1}/${chunks.length} (${chunks[i].length} chars)`);
    ...
    async searchSimilar(query, businessId, topK = 5) {
        try {
            ...
            const searchResponse = await RetryManager.withRetry(async () => {
            ...
            if (searchResponse.matches.length > 0) {
                searchResponse.matches.forEach((match, i) => {
                    logger.debug(`[VECTOR] Match ${i + 1}: Score=${match.score.toFixed(3)}, File=${match.metadata.filename}, Content="${match.metadata.content?.substring(0, 100)}..."`);
                });
            } else {
    ...
    async deleteDocument(businessId, filename) {
        try {
            ...
            if (vectorIds.length === 0) {
                logger.warn(`[VECTOR] No vectors found to delete for ${filename} in business ${businessId}`);
            ...
            logger.success(`[VECTOR] Successfully deleted ${vectorIds.length} vector chunks for ${filename}`);
    ...
    async deleteByKnowledgeId(knowledgeId) {
        try {
            ...
            searchResponse.matches.forEach(match => {
                if (match.id.includes(knowledgeId) || 
                    (match.metadata && match.metadata.filename && match.metadata.filename.includes(knowledgeId))) {
                    vectorIds.push(match.id);
                }
            });
            ...
            logger.info(`[VECTOR] Found ${vectorIds.length} vector chunks to delete for knowledge ${knowledgeId}`);
            ...
            logger.success(`[VECTOR] Successfully deleted ${vectorIds.length} vector chunks for knowledge ${knowledgeId}`);
            return { success: true, deletedCount: vectorIds.length };
