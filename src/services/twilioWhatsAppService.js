...
class TwilioWhatsAppService {
    ...
    async registerBusiness(businessId, businessName, whatsappNumber, ownerPhone) {
        try {
            logger.info(`[TWILIO] Registering business ${businessName} with WhatsApp number ${whatsappNumber}`);
            ...
            logger.success(`[TWILIO] Successfully registered business ${businessName} (${businessId}) with WhatsApp ${whatsappNumber}`);
    ...
    async sendMessage(to, message, fromNumber = null) {
        try {
            ...
            logger.debug(`[TWILIO] Sending message from ${from} to ${to}: "${message.substring(0, 50)}..."`);
