...
class WebhookHandler {
    ...
    async handleTwilioWebhook(req, res) {
        try {
            ...
            if (process.env.NODE_ENV === 'production' && !twilioWhatsAppService.validateWebhookSignature(signature, url, req.body)) {
            ...
            if (!rateLimiter.checkGlobal()) {
                ...
                await this.sendResponse(from, to, '⚠️ System is experiencing high load. Please try again in a few minutes.');
    ...
    async routeMessage(messageData) {
        ...
        try {
            ...
            if (!business) {
                ...
                await this.sendResponse(from, to, 'This WhatsApp number is not associated with any business. Please contact support.');
                return;
            }

            logger.debug(`[WEBHOOK] Routing message to business: ${business.businessName} (${business.businessId})`);
            ...
            await this.sendResponse(from, to, 'Sorry, I encountered an error processing your message. Please try again.');
        }
    }
    ...
    async handleBusinessOwnerMessage(messageData, business) {
        ...
        try {
            ...
            if (parsedCommand) {
                ...
                await this.sendResponse(from, to, 'Command not recognized. Send !help for available commands.');
            }

        } catch (error) {
            logger.error(`[WEBHOOK] Error handling business owner message from ${business.businessName}:`, error);
    ...
    async handleCustomerMessage(messageData, business) {
        ...
        try {
            ...
            if (!rateLimiter.checkCustomer(phoneNumber)) {
                ...
                await this.sendResponse(from, to, `⚠️ Rate limit exceeded. You can make ${remaining} more queries. Limit resets at ${resetTime.toLocaleTimeString()}.`);
                return;
            }
            ...
            logger.error(`[WEBHOOK] Error handling customer message for ${business.businessName}:`, error);
            await this.sendResponse(from, to, 'Sorry, I couldn\'t find information about that. Please try rephrasing your question.');
        }
    }
    ...
    async processBusinessOwnerCommand(parsedCommand, messageData, business) {
        ...
        try {
            ...
            if (!rateLimiter.checkBusinessOwner(business.ownerPhone)) {
                ...
                await this.sendResponse(from, to, `⚠️ Rate limit exceeded. You can make ${remaining} more requests. Limit resets at ${resetTime.toLocaleTimeString()}.`);
                return;
            }
            ...
            switch (command) {
                ...
                default:
                    await this.sendResponse(from, to, `Unknown command: ${command}. Send !help for available commands.`);
            }
    ...
    async handleAddKnowledge(args, messageData, business) {
        ...
        try {
            if (!args.trim()) {
                await this.sendResponse(from, to, 'Please provide content. Format: !add [your knowledge text]');
                return;
            }
            ...
            if (validator.detectSuspiciousContent(contentValidation.sanitized)) {
                await this.sendResponse(from, to, '❌ Content contains suspicious elements and cannot be processed.');
                return;
            }
    ...
    async handleListKnowledge(messageData, business) {
        ...
        try {
            ...
            if (entries.length === 0) {
                await this.sendResponse(from, to, `📚 Your Knowledge Base (${business.businessName}):\n\nNo entries yet. Use !add [text] or send documents to get started!`);
                return;
            }
    ...
    async handleDeleteKnowledge(args, messageData, business) {
        ...
        try {
            if (!args.trim()) {
                await this.sendResponse(from, to, 'Please provide a knowledge ID. Format: !delete [knowledge-id]');
                return;
            }
            ...
            const result = await knowledgeService.deleteKnowledge(business.businessId, idValidation.sanitized);
...
- Use descriptive text for better customer responses`;
    ...
    async handleClearCache(messageData, business) {
        ...
        try {
            ...
            await this.sendResponse(from, to, `🗑️ Cleared ${clearedCount} cached responses for ${business.businessName}. Fresh queries will now search updated knowledge base.`);
    ...
    async handleMediaUpload(messageData, business) {
        ...
        await this.sendResponse(from, to, '📄 Media upload support coming soon! For now, please use !add [text] to add knowledge.');
    }
